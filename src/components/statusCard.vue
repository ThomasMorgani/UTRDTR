  ...
  computed: {
      ...
      switch (this.gameData.round) {
        ...
        case 5:
          ...
          if (this.gameData.roundPhase >= 1) {
            ...
            statusData.text = `${action.toUpperCase()} ${this.gameData
              .roundPhase * 2}: Players with ${card.card}`
          } else {
    ...
    statusMessage() {
      ...
      if (this.gameData.round === 5) {
        if (!this.gameData.givingDrinks && !this.gameData.drinksGiven) {
...
<template>
  <div class="uk-card uk-card-default uk-backgrond-muted uk-padding">
    <p v-if="isLoading">LOADING....</p>
    <div class="uk-flex uk-flex-column uk-flex-center uk-flex-middle" v-else>
      <div class="uk-text-lead uk-text-primary uk-text-bold">
        {{ gameStatus.title }}
      </div>
      ...
      <template v-if="!isLoading && gameData.round < 5">
        <transition
          ...
          mode="out-in"
        >
          <img :key="deckImage" class="ui image deckImg" :src="deckImage" />
        </transition>
      </template>
      <div v-else class="uk-margin-top uk-width-1-1">
        <transition-group
          name="fadeInLeftBig"
          tag="div"
          appear
        ...
        >
          <div class="column" v-for="n in 4" :key="'rvrup' + n">
        ...
        <transition-group
          ...
          class="uk-flex uk-flex-between"
        >
          <div class="column" v-for="n in 4" :key="'rvrdwn' + n">
            <transition name="flip">
              <img
                :key="
      ...
      <h4 v-else v-text="statusMessage" class="center aligned"></h4>
    </div>
