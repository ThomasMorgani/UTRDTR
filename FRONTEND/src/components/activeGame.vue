...
export default {
  ...
  components: {
    ...
    playersList,
    statusCard,
    update: () => import('vue-material-design-icons/Update'),
    user: () => import('vue-material-design-icons/Account')
  },
  props: {
    passedGameData: {
      type: Object,
  ...
  computed: {
    ...
    modalData() {
      ...
      if (this.gameData.round <= 4) {
        ...
        drinksCount = this.gameData.cardsRiver.give.length * 2
      }
      return {
  ...
  methods: {
    ...
    settingsUpdate() {
      ...
      this.setStateValue({ key: 'settings', val: { ...settings } })
    },
    setTurnTimer(time) {
    ...
    toggleTabView() {
      this.tabView = this.tabView === 'playersList' ? 'logList' : 'playersList'
    }
  },
  async created() {
...
<template>
  <div class="uk-flex uk-flex-column  uk-width-expand">
    ...
    <!-- TODO: refactor this!! -->
    <!-- load views here -->
    <div class="uk-flex uk-flex-around" v-if="!settings.isMobile">
      <div class="uk-flex-1">
        <statusCard
      ...
      <div class="  playersCol  uk-flex uk-flex-center">
        <playersList @viewCard="onViewCard"></playersList>
      </div>
    </div>
    <!-- mobile view -->
    <div class="uk-flex uk-flex-column uk-justify-start" v-else>
      <div class="uk-width-1-1 uk-flex-none">
        <statusCard
          @submitAction="onSubmitAction"
          @viewCard="onViewCard"
        ></statusCard>
      ...
      <ul
        uk-tab
        class="uk-flex-center uk-child-width-expand   uk-width-1-1"
        v-if="!isLoading"
      >
        <li
          class=" uk-text-center"
          :class="{ 'uk-active': tabView === 'logList' }"
          @click="tabView = 'logList'"
        >
          <a> <update title="HISTORY"></update></a>
        </li>
        <li
          class="uk-text-center"
          :class="{ 'uk-active': tabView === 'playersList' }"
          @click="tabView = 'playersList'"
        >
          <a> <user title="PLAYERS"></user></a>
        </li>
      </ul>
      <component
        :is="tabView"
        ...
        @viewCard="onViewCard"
        :class="
          `animation: uk-animation-slide-${
            tabView === 'logList' ? 'right' : 'left'
          }-medium, uk-animation-slide-${
            tabView === 'logList' ? 'left' : 'right'
          }-medium`
        "
        class="uk-width-1-1"
      ></component>
